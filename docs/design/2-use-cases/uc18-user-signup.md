...
| **Business Rules** | **Rule 18.1**: Password MUST NEVER reach the server in plain text (Derived Auth Token used).<br>**Rule 18.2**: Registration flow MUST include visual progress (loading state).<br>**Rule 18.3**: Redirection to `/notes/` MUST occur only after successful vault initialization. |
