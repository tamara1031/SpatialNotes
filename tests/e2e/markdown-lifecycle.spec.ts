...
import { getUniqueNodeId } from "./utils/auth";
...
test.describe("Markdown Notebook Lifecycle", () => {
	test("Signup, Create Markdown Notebook, Edit, and Delete", async ({ page }) => {
		...
		// 7. Edit content
		await editor.click();
		await page.keyboard.type("# Welcome to ProseMirror\n\nThis is a test paragraph.");
		...
		await expect(page.locator(`.sidebar >> text=${notebookName}`)).toBeVisible();

		// 10. Delete Notebook
		await page.click(`.sidebar >> text=${notebookName}`); // Select again to show delete icon
		await page.click('button[title="Delete node"]');
		await expect(page.locator(`.sidebar >> text=${notebookName}`)).not.toBeVisible();
	});
});
