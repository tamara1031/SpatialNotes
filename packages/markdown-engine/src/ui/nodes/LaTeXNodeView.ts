...
export class LaTeXNodeView implements NodeView {
	...
	constructor(node: Node, view: EditorView, getPos: () => number | undefined) {
		...
		this.dom.className =
			"latex-node-view py-2 my-2 cursor-pointer transition-colors hover:bg-zinc-800/50 rounded";
	...
	render() {
		...
		if (this.isRendered && this.node.textContent.trim()) {
			...
			editor.className =
				"p-4 font-mono text-sm text-emerald-400 bg-zinc-950/50 rounded border border-zinc-700/50";
