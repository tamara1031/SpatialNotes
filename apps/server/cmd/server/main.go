...
func main() {
	...
	mux.Handle("GET /api/nodes/{id}", authMiddleware(http.HandlerFunc(nodeHandler.HandleGet)))
	mux.Handle("PATCH /api/nodes/{id}", authMiddleware(http.HandlerFunc(nodeHandler.HandleMove)))
